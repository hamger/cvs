...
import {isPointInPath, getPointAtLength, getTotalLength} from './deal'
...
export default class SvgPath {
  constructor (d) {
    if (!isSvgPath(d)) throw new Error('Not an SVG path!')
  ...
  to (context) {
    ...
    if (commands.length) {
      if (this[_beginPath]) context.beginPath()
      commands.forEach((c) => {
        const [cmd, ...args] = c
        if (cmd === 'M') {
          context.moveTo(...args)
      ...
      if (this[_initialPath][this[_initialPath].length - 1][0] === 'Z') context.closePath()
    }
