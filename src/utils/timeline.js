// borrow from sprite-timeline (https://github.com/spritejs/sprite-timeline)
...
class Timeline {
  ...
  get entropy () {
    ...
    return (
      entropy +
      Math.abs((this.globalEntropy - globalEntropy) * this.playbackRate)
    )
  }
  ...
  [_setTimer] (handler, time, id = Symbol('timerID')) {
    ...
    if (timer) {
      ...
      if (time.isEntropy) {
        delay =
          (time.delay - (this.entropy - timer.startEntropy)) /
          Math.abs(this.playbackRate)
      } else {
        delay =
          (time.delay - (this.currentTime - timer.startTime)) /
          this.playbackRate
      }
      ...
      delay =
        time.delay /
        (time.isEntropy ? Math.abs(this.playbackRate) : this.playbackRate)
